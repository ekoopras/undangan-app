...
<section id="gift" class="h-[100dvh] w-full flex flex-col justify-between items-center text-center py-12 px-6 bg-transparent relative snap-start shrink-0 box-border overflow-hidden">
    ...
    <div class="my-auto w-full max-w-xs space-y-5">
        ...
        <p class="font-inter text-[11px] leading-relaxed text-[#0c4643] drop-shadow-[0_4px_12px_rgba(0,0,0,0.4)] px-2">
        ...
        <div class="p-4 bg-white/10 border border-[#0c4643]/20 rounded-2xl backdrop-blur-md shadow-md flex flex-col items-center">
            <!-- Logo Bank / Nama Bank -->
            <span class="font-inter text-xs font-bold tracking-widest text-[#0c4643] uppercase mb-1">
                {{ $item['nama_bank'] ?? 'Bank' }}
            </span>

            <!-- Nomor Rekening -->
            <p class="font-inter text-sm font-black text-[#0c4643] tracking-wider mb-1" id="norek-1">
                {{ $item['no_rekening'] ?? '000000000' }}
            </p>

            <!-- Nama Pemilik Rekening -->
            <p class="font-inter text-[10px] text-[#0c4643]/85 uppercase tracking-wide mb-3">
                {{ $item['atas_nama'] ?? '-' }}
            </p>
